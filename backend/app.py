...
def get_metrics(time_range):
    ...
    if metrics["cpu_user"][-1]["os_type"] != "windows":
      ...
      def classify_system(cpu_user, cpu_system, cpu_idle, mem_used_percent, swap_used_percent, load1, load5, load15, disk_used_percent, drop_in, drop_out):
      ...
      df["system_label"] = df.apply(
          lambda row: classify_system(metrics["cpu_user"][-1]["value"], metrics["cpu_system"][-1]["value"], metrics["cpu_idle"][-1]["value"], metrics["mem_used_percent"][-1]["value"], metrics["swap_used_percent"][-1]["value"], metrics["load1"][-1]["value"], metrics["load5"][-1]["value"], metrics["load15"][-1]["value"], metrics["disk_used_percent"][-1]["value"], metrics["drop_in"][-1]["value"], metrics["drop_out"][-1]["value"]),
          axis=1
      )
        
      filename = "ml_datasets/host_datasets/system-metrics-" + hostname + ".csv"
      ...
      model = joblib.load("ml_models/random_forest_model_linux.pk1")
      le = joblib.load("ml_models/label_encoder_linux.pk1")
      scaler = joblib.load("ml_models/standard_scaler_linux.pk1")

      original_df = pd.read_csv("ml_datasets/main_datasets/system-metrics-linux.csv", sep=",")
      yeni_df = pd.read_csv("ml_datasets/host_datasets/system-metrics-" + hostname + ".csv")

      if "model_tahmini" not in yeni_df.columns:
          yeni_df["model_tahmini"] = None


      tahmin_yapilacak = yeni_df[yeni_df["model_tahmini"].isna()]

      if not tahmin_yapilacak.empty:
          X_yeni = tahmin_yapilacak[["cpu_user", "cpu_system", "mem_used_percent", "swap_used_percent", "disk_used_percent", "diskio_ms", "drop_in", "drop_out", "load1", "load5", "load15"]]

            # Eksik veri kontrolü
          if X_yeni.isnull().any().any():
              raise ValueError("Tahmin verisinde eksik değerler bulunuyor!")

          X_yeni_scaled = scaler.transform(X_yeni)


          y_pred = model.predict(X_yeni_scaled)


          etiketler = list(le.classes_)
          print("Sayısal Tahmin: ", y_pred[-1])
          print("Etiket Tahmin: ", etiketler[y_pred[-1]])
          y_label = le.inverse_transform(y_pred)
          yeni_df.loc[yeni_df["model_tahmini"].isna(), "model_tahmini"] = y_label
          metrics["model_tahmin"] = y_label[-1]

            
          yeni_df.to_csv("ml_datasets/host_datasets/system-metrics-" + hostname + ".csv", index=False)

        #['boşta' 'kritik uyarı' 'normal' 'yüksek uyarı']
        ...
        model = joblib.load("ml_models/random_forest_model.pk1")
        le = joblib.load("ml_models/label_encoder.pk1")
        scaler = joblib.load("ml_models/standard_scaler.pk1")

        original_df = pd.read_csv("ml_datasets/main_datasets/system-metrics-windows.csv", sep=",")
        yeni_df = pd.read_csv("ml_datasets/host_datasets/system-metrics-" + hostname + ".csv")
        ...
        if not tahmin_yapilacak.empty:
            X_yeni = tahmin_yapilacak[["cpu_user", "cpu_system", "mem_used_percent", "swap_used_percent", "disk_used_percent", "diskio_ms", "drop_in_received", "drop_out_outbound", "load1", "load5", "load15"]]
    ...
    if win_result["hits"]["hits"] and "windows" in win_result["hits"]["hits"][0]["_source"]["tags"]:
        ...
        if linux_result["hits"]["hits"] and "linux" in linux_result["hits"]["hits"][0]["_source"]["tags"]:
            for hit in linux_result["hits"]["hits"]:
                ...
                if agent_type == "filebeat" and "linux" in tags and "container" in tags:
                    linuxlogs.append({
                        "timestamp": hit["_source"].get("@timestamp"),
                        "host": hit["_source"]["host"].get("name", "N/A"),
                        "program": hit["_source"]["service"].get("type", ""),
                        "programID": hit["_source"].get("process.thread.name", ""),
                        "message": hit["_source"].get("message", "")
                    })

            return jsonify(linuxlogs)

    return jsonify(["error"]) # Boş bir liste veya uygun bir mesaj dönebilirsin.
